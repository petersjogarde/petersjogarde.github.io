(function(global) {
  ...
  function d3network(data, options) {
    ...
    let simulation = d3.forceSimulation()
      ...
      .force('collide', d3.forceCollide().radius(d => d.size + 1).iterations(1));
    ...
    function min_max_node_size() {
      ...
      Array.prototype.forEach.call(nodeSliders, slider => {
        ...
        slider.previousElementSibling.innerText = defaultValue;
    ...
    function hoverNode(event) {
      ...
      let radius = (target.size * node_size_scalar * tScale) / transform.k;
    ...
    function simulationUpdate(returnNeighbours) {
      ...
      data.links.forEach(d => {
        if (d.weight >= edgethreshold_size_scalar) {
          if (hiddenNodes.indexOf(d.source.index) === -1 && hiddenNodes.indexOf(d.target.index) === -1) {
            ...
            if (edge_size_scalar) {
              context.lineWidth = d.weight ? (d.weight / 2 * edge_size_scalar) : 5;
              context.stroke();
            }
      ...
      data.nodes.forEach(d => {
        ...
        if (hiddenNodes.indexOf(d.index) === -1) {
          context.beginPath();
          context.arc(transform.applyX(d.x), transform.applyY(d.y), (d.size ? d.size : radius) * node_size_scalar * tScale, 0, 2 * Math.PI, true);
          context.fillStyle = d.color ? d.color : 'black';

          if (d === selectedNode) {
            context.beginPath();
            context.arc(transform.applyX(d.x), transform.applyY(d.y), ((d.size ? d.size : radius) * node_size_scalar * tScale) - 1 * tScale, 0, 2 * Math.PI, true);
      ...
      data.nodes.slice(0).sort((a, b) => b.size - a.size).forEach(d => {
        //labels
        if (d.size >= labelthreshold_size_scalar && hiddenNodes.indexOf(d.index) === -1) {
          ...
          let uy = transform.applyY(d.y) - (fontSize / 1.75) - (d.size ? d.size : radius) * node_size_scalar * tScale;
          //collisions?
      ...
      if (hoveredNode && hiddenNodes.indexOf(hoveredNode.index) === -1) {
        ...
        context.fillRect((transform.applyX(hoveredNode.x) - textWidth / 2) - scaleWidth, transform.applyY(hoveredNode.y) - scaleHeight - (hoveredNode.size ? hoveredNode.size : radius) * node_size_scalar * tScale, textWidth + (scaleWidth * 2), fontSize);
        context.fillStyle = '#FFFFFF';
        context.fillText(hoveredNode.label, Math.round((transform.applyX(hoveredNode.x) - textWidth / 2)), transform.applyY(hoveredNode.y) - (fontSize / 1.75) - (hoveredNode.size ? hoveredNode.size : radius) * node_size_scalar * tScale);
      }
