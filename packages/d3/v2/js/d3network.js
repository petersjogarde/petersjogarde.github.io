(function(global) {
  ...
  function d3network(data, options) {
    ...
    let zoomOption = options && options.zoom ? options.zoom : [1 / 10, 8],
      ...
      edgeScale = (weight) => {
		return (weight / edgeSizes[edgeSizes.length - 2]) * (options && options.largestNode ? options.largestNode : 30) * (options && options.largestEdge ? options.largestEdge : 0.5);
      };
    ...
    function min_max_node_size() {
      ...
      nodeSizes.push(nodeSizes[nodeSizes.length - 1]);
      edgeSizes.sort((a, b) => a - b);
      edgeSizes.push(edgeSizes[edgeSizes.length - 1]);
